...
# An attacker can force the prize vault into a state such that `withdraw` will revert for all users that deposited

## Summary
An attacker can force the prize vault into a state such that `withdraw` will revert for all users that deposited

It involves liquidating a large amount of yield such that the remaining amount of yieldVault shares held by the prizeVault = 1, now when the attacker withdraws yieldVault shares held by the prizeVault = 0.

At this point calls to `withdraw` will revert 

Skip to the **Proof of concept** section for a fully coded walkthrough


## Vulnerability Detail
**The vulnerability uses 2 main ideas**
1. A large liquidation of yield can cause yieldVault shares owned by the prizeVault = 1, then withdrawing to make yieldVault shares = 0
2. If yieldVault shares owned by the prizeVault = 0, calls to `withdraw()` will revert due to using `totalPreciseAssets()`

Diving into this deeper

**Q: Why will calls to `withdraw()` revert IF yieldVault shares owned by the prizeVault = 0**
**TLDR:** If yieldVault shares owned by the prizeVault = 0, `totalPreciseAssets()` will revert

Taking a look at `PrizeVault.withdraw()` flow

`PrizeVault.withdraw()` ==> `previewWithdraw()` ==> `totalPreciseAssets()`
...
This function may revert for reasons that would cause `yieldVault.previewRedeem` to revert, as seen in the natspec

One example when it will revert is when yieldVault shares = 0, this is because a lot of vaults will have the following sanity check

```solidity
require(shares > 0, "Shares must be greater than 0");
```

Some more proof of this behaviour is from the EIP-4626 itself, that states the following
...
## Impact
Withdraws will revert for all users of the vault.

Funds stuck for all users of the vault.

Large distrust will be created against the protocol if a user's withdraw reverts, especially considering the protocol is meant to be a savings protocol.

Since yieldVault shares = 0, no yield will accumulate for these user's deposits
...
Now add the following ComplexWithdrawDOSPOC.sol to /2024-05-pooltogether/pt-v5-vault/test/integration/ComplexWithdrawDOSPOC.sol

The following foundry test test__ComplexWithdrawDOSPOC() illustrates the attack scenario.

Run it with the following command line input

```terminal
forge test --mt test__ComplexWithdrawDOSPOC -vv
```
...
contract ComplexWithdrawDOSPOC is Test, BaseIntegration {
    ...
    function test__ComplexWithdrawDOSPOC() public {
        ...
        // When the other users try to withdraw it will revert
        vm.startPrank(vitalik);
        uint256 vitalikBalance = prizeVault.balanceOf(vitalik);
        vm.expectRevert("Shares must be greater than 0");
        prizeVault.withdraw(vitalikBalance, vitalik, vitalik);

        vm.startPrank(alice);
        uint256 aliceBalance = prizeVault.balanceOf(alice);
        vm.expectRevert("Shares must be greater than 0");
        prizeVault.withdraw(aliceBalance, alice, alice);
    }
...
```terminal
Ran 1 test for test/integration/ComplexWithdrawDOSPOC.sol:ComplexWithdrawDOSPOC
[PASS] test__ComplexWithdrawDOSPOC() (gas: 622655)
Logs:
...
Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 2.35ms (1.06ms CPU time)

Ran 1 test suite in 14.62ms (2.35ms CPU time): 1 tests passed, 0 failed, 0 skipped (1 total tests)
```

Note the attacker can perform all their steps in 1 Tx therefore can use a flashloan if needed


## Code Snippet
https://github.com/sherlock-audit/2024-05-pooltogether/blob/1aa1b8c028b659585e4c7a6b9b652fb075f86db3/pt-v5-vault/src/PrizeVault.sol#L1059-L1060
...
## Recommendation
Possibly using `_tryGetTotalPreciseAssets` in `previewWithdraw`, I did not have the time to verify if this works / is foolproof
