...
```terminal
forge test --mt test__FrontrunToLoseYield -vv
```
...
contract FrontrunToLoseYield is Test, BaseIntegration {
    ...
    function test__FrontrunToLoseYield() public {
        ...
        // The attacker sees an incoming yield Tx to the vault in the mempool, he will frontrun that Tx with the following Tx
        ...
        // Yield Accrual Tx goes through BUT since the prizeVault has 0 shares they will not be credited to the PrizeVault users
        _accrueYield();

        // At this point if the attacker own's his own yieldVault shares he would have stolen all the yield that was meant to be shared with the vault users too
    }
...
```terminal
Ran 1 test for test/integration/FrontrunToLoseYield.sol:FrontrunToLoseYield
